...
Запускайте основной сценарий командой `sudo ./scripts/main.sh`; он создает и настраивает `arch_root.img`. Перед коммитом выполняйте `bash -n scripts/*.sh lib/*.sh tests/*.sh` для быстрой проверки синтаксиса всех поддерживаемых shell-файлов. Если установлен `shellcheck`, запускайте `shellcheck scripts/*.sh lib/*.sh tests/*.sh`, чтобы поймать проблемы с quoting, `source` и переносимостью. Для проверки CI-контура собирайте builder-образ через `docker build -t rpi5-archlinux-builder .`. Релизный workflow публикует `arch_root.img.xz`, а не сырой `.img`, из-за лимита GitHub Release на размер asset. Проверять итоговый образ удобно стандартными системными утилитами, например `losetup -fP arch_root.img` и `mount`.
