...
- `dist/images/` — generated каталог для локальных `archlinuxarm-rpi5-aarch64.img` и `archlinuxarm-qemu-aarch64.img`, boot-файлов QEMU в `qemu-boot/` и release artifacts вида `archlinuxarm-rpi5-aarch64-${TAG}.img.xz` / `archlinuxarm-qemu-aarch64-${TAG}.img.xz`; каталог `dist/` не коммитится.
...
QEMU target builds `dist/images/archlinuxarm-qemu-aarch64.img` and exports direct-boot files to `dist/images/qemu-boot/`. The runner uses `qemu-system-aarch64 -M virt` with virtio disk, serial console, user networking, and host SSH forwarding from `localhost:2222` to guest port `22`.
