...
[[ -n "$_LIB_BOOTSTRAP_LOADED" ]] && return || readonly _LIB_BOOTSTRAP_LOADED=1

# Импорт зависимостей
source "$(dirname "${BASH_SOURCE}")/log.sh"
...
bootstrap::install_base() {
    ...
    if pacstrap -C ./conf/pacman-arm.conf -M -K "$target" "${pkgs[@]}" --noconfirm; then
